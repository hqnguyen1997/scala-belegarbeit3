...
object Utils {
  ...
  def countAllMissingValues(passengers: List[Map[String, Any]], attList: List[String]): Map[String, Int] = {
    passengers.foldLeft(Map[String, Int]())((base, passenger) => {
      attList.foldLeft(base)((base, att) => {
        if (passenger.getOrElse(att, null) == null)
          base.updated(att, base.getOrElse(att, 0) + 1)
        else base
      })
    })
  }
